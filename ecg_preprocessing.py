...
def signal_quality_in_window(qrs1, qrs5, QRS_Pos_Err, fs):
    
    if qrs1 is not None and qrs5 is not None:
        if len(qrs1) <= len(qrs5):
            length = len(qrs1)
            short_qrs = qrs1.copy()
            long_qrs = qrs5.copy()
        else:
        ...
        for k in range(0,length):
            dis = np.sort(abs(long_qrs-short_qrs[k]))
            # dis.sort()
            if dis[0] / fs < QRS_Pos_Err:
                cn = cn + 1
        
        wsqi = cn / (len(qrs1) + len(qrs5) - cn)
...
def signal_quality_in_window_based_on_waveform(ecg_reversed, qrs, fs):
        ecg_signal = ecg_reversed.copy()  
        cycle_num = len(qrs) - 1
        TD = fs
        equal_D_cycles = np.zeros((cycle_num,TD))
        
        for cn in range(0,cycle_num):
            temcycle = ecg_signal[qrs[cn]:qrs[cn+1]     ]   ########
            tem_cd = len(temcycle)
            time_scale_ratio = TD / tem_cd            
            equal_D_cycles[cn,0 : TD] = interpolate.pchip_interpolate(np.arange(tem_cd) * time_scale_ratio, temcycle, np.arange(TD))
            temcycle = 0
            
        if (cycle_num == 1):
            mean_cycle = equal_D_cycles
        else:
            mean_cycle = sum(equal_D_cycles) / cycle_num
         
        err=[]
        for cn in range(0,cycle_num):
            err_i = sum((mean_cycle - equal_D_cycles[cn, : ]) ** 2)
            err.append(err_i)
        err = np.array(err)
        cycle_energy = sum((mean_cycle ** 2).flatten())
        
        if ( np.mean(err) >= 0.5 * cycle_energy):
            wsqi_w = 0
        else:
            wsqi_w = 1
...
def signal_quality_est_ver2(ecg_reversed, fs, QRS_Pos_Err, Win_len, Sliding_step, Signal_Quality_TH):
    
    rpeaks = ecg.christov_segmenter(ecg_reversed, sampling_rate=fs)
    rpeaks_indices_1 = rpeaks[0]
    correct_tol = 0.05
    rpeaks_correct = ecg.correct_rpeaks(ecg_reversed, rpeaks_indices_1, fs, tol=correct_tol)
    rpeaks_indices_1 = rpeaks_correct[0]
    r_peaks2 = Rwave_Detection(ecg_reversed, fs=200)
    r_peaks2=np.array(r_peaks2,dtype="int32")    
    qrs1 = r_peaks2
    qrs5 = rpeaks_indices_1
   
    ecg_signal = ecg_reversed.copy()
    orN = len(ecg_signal)
    rem = np.mod(len(ecg_signal),Win_len)  
    if rem > 0 :
        ecg_signal = np.append(ecg_reversed,np.zeros(Win_len-rem))
    sqi_flag = np.ones(len(ecg_signal))
    
    for k in range(0, len(ecg_signal)-Win_len, Sliding_step):
        win_ecg_ind = np.arange(k, k + Win_len)        
        win_qrs1_ind = np.where( (qrs1>=np.min(win_ecg_ind)) & (qrs1<=np.max(win_ecg_ind)) )
        win_qrs5_ind = np.where( (qrs5>=np.min(win_ecg_ind)) & (qrs5<=np.max(win_ecg_ind)) )
        
        wsqi = signal_quality_in_window(qrs1[win_qrs1_ind], qrs5[win_qrs5_ind], QRS_Pos_Err, fs)
        wsqi_w = signal_quality_in_window_based_on_waveform(ecg_signal, qrs1[win_qrs1_ind], fs)
        
        if wsqi<Signal_Quality_TH and wsqi_w==0 and k<len(ecg_signal)-Win_len:
            sqi_flag[k:k+Sliding_step] = 0
        elif wsqi<Signal_Quality_TH and wsqi_w==0 and k>=len(ecg_signal)-Win_len:
                sqi_flag[win_ecg_ind] = 0
    
    tem_sqi_flag = sqi_flag[:orN]
    sqi_flag = tem_sqi_flag
