...
def challenge_entry(sample_path,model1):
   ...
   # tmp_pre.insert(0,tmp_pre[0])
    ...
    if result < 0.1 and issubarray(np.array(np.round(predict)),np.array([1,1,1,1,1]))<10:
        test_result = 0;
    elif result > 0.9 and issubarray(np.array(np.round(predict)),np.array([0,0,0,0,0]))<10:
        test_result = 1;
    else:
        test_result = 2;    
    end_points = []
